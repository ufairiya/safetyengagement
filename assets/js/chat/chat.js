$(function() {
$('.messagedataval').on('mouseup keypress',function(event){
    var keycode = (event.keyCode ? event.keyCode : event.which);

    if(keycode == '13'){
       sendTxtMessage($('.messagedataval').val());
    }
          var receiver_id = $('.comid').val();
...
ChatSection(1);
	  ...
	  GetChatHistory(receiver_id,poid);
 				
$('.upload_attachmentfile').on('change',function(){
	...
	var Sender_ProfilePic = $('#Sender_ProfilePic').val();
		...
		var str = '<div class="direct-chat-msg right">';
				str+='<div class="direct-chat-info clearfix">';
				 ...
				 str+='</div><img title="'+Sender_Name+'" class="direct-chat-img" src="'+Sender_ProfilePic+'" alt="">';
		...
		//console.log(message);
 		DisplayMessage(messageTxt);
				...
				$.ajax({
						  dataType : "json",
						  type : 'post',
						  data : {messageTxt : messageTxt, receiver_id : receiver_id,poid : poid },
...
function updatestatusMessage(){
	...
	$.ajax({
			  dataType : "json",
			  type : 'post',
			 data : { Sender_Id : Sender_Id,poid : poid },
			url: baseurl+'ChatController/updatestatusmessage',
			success:function(data)
			{
	  ...
	  if(Sender_Name == "")
		{
				...
				$.ajax({
						  //dataType : "json",
  						  url: baseurl+'get-chat-history-vendor?receiver_id='+receiver_id+'&poid='+poid,
						  success:function(data)
						  {
							
					

  						$('#frame .messages ul').html(data);
	...
	function GetChatcount(){
		var baseurl = $('.baseurl').val();
				...
				$.ajax({
						   dataType : "json",
							type : 'post',
  						  url: baseurl+'ChatController/getchatcountbadge',
						  success:function(data)
						  {
							
						console.log(data);
  						$('#badgecountid').html('');
  						$('#badgecountid').append(data);
						//	ScrollDown();
