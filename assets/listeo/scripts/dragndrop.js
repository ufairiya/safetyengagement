(function()
{
	...
	document.addEventListener('dragend', function(e)
	{
		...
		if(selections.droptarget)
		{
			...
			for(var len = selections.items.length, i = 0; i < len; i ++)
			{
				var taskid =  $(selections.items[i]).attr('data_taskid');
				var userid =  $(selections.droptarget).find('.user_id').val();			  
				var device_id =  $(selections.droptarget).find('.device_id').val();			  
				selections.droptarget.appendChild(selections.items[i]);
				
				//~ console.log('tidsurya:'+taskid);
			 //~ console.log('uidsurya:'+device_id);				
		...
		if(taskid != '')
		{
			var schebasegeturl = $('.schebasegeturl').val();
		jQuery.ajax({
				url : schebasegeturl+'admin/schedule/addschedule_temp',
				type: 'POST',
				data: {'taskuserid':taskuserid,'device_id':device_id},
					success:function(response){
						 $("#bookingdatefilter_"+taskid).removeClass("pending-booking");
						  $("#bookingdatefilter_"+taskid).addClass("assigning-booking");
						   $(".assigning-booking .bookings .inner h3 span.booking-status").css('background-color', '#40e0d0');
						//~ toastr.options = {
						//~ "closeButton": true,
						//~ }
