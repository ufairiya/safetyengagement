                  ...
                  <div class="portlet light portlet-fit portlet-datatable ">
                    ...
                    <div class="portlet-body">
                      <div class="row">
                        ...
                        <div class="row">
                        <div class="col-md-12"> 
                         <label class="col-md-3">Select User :</label>
                         <div class="col-md-6">
                         <?php 
                          $userAttr = array(
                            'class'=> 'form-control',
                          ...
                          echo form_dropdown('user_id',$users,'',$userAttr);
                         ?>
                         </div>
                          </div>
                        </div>
                        <div style="margin-bottom: 20px;"></div>
                          ...
                          <input type="checkbox" class="ckbox" id="data_'.$v->id_access_privileges.'" value="'.$v->id_access_privileges.'" name="'.$user_types_custom_values[$i].'[]" '.$selected.'>  <span></span></label></td>';
													}
													echo "</tr>";
												}
...
jQuery(document).ready(function(e) {
	jQuery(document).on("click","#save_user_access",function(event){
		...
		jQuery.ajax({
		 type: "POST",
		 url: "<?php echo $base_url;?>admin/user/save_user_access", 
		 data: form.serialize(),
		 success: 
			  function(data){
...
jQuery(document).on('change','#user_id',function(){
  var uid = jQuery('#user_id').val();
    jQuery.ajax({
     type: "POST",
     url: "<?php echo $base_url;?>admin/user/user_privileges", 
     data: {'userId':uid},
     dataType:'JSON',
     success:function(data){         
         jQuery('.ckbox').prop('checked',false);
         if(data.list != false){
            $.each(data.list, function (i) {
              var userPrev = data.list[i];
               var idPrev = userPrev.id_privileges;              
               jQuery('#data_'+idPrev).prop('checked',true);
            });
         }
        }
      });
