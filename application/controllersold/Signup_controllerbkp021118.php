...
$company_name = ($this->input->post('company_name')) ? $this->input->post('company_name') : '';
	if(empty($this->input->post('registration_type'))) {

		$comp_sf = ($this->input->post('student')) ? $this->input->post('student') : 'professional';
	}
	else
	{
		$comp_sf =$this->input->post('registration_type');
	}
	if(empty($this->input->post('sfemail')))
	{
		$compsfemail = $this->input->post('jobemail');
	}else{
				$compsfemail = $this->input->post('sfemail');
...
$lastid =$this->Employee_Model->insertEmployee($data);
			//~ if($lastid != FALSE){				
				//~ $user_address = array(
					...
					//~ 'useraddress_email'=>$this->input->post("txt_email"),
		  ...
		  if($lastid){           
			//send confirm mail
			 //~ if($this->Employee_Model->sendEmail($this->input->post('txt_email'),$this->input->post('jobfirst'),$lastid))
			$this->load->library('email');
...
{			
	 ...
	 else
	{
			 print json_encode(array('status'=>'failed'));
							...
							//~ //echo '<div class="alert alert-danger text-center">Failed!! Please try again.</div>';                  
    ...
    function updateprofile()
    {
			...
			//~ echo '</pre>';
			//~ if((!empty($this->input->post('txt_new_pass'))) && ((!empty($this->input->post('txt_new_pass'))) )
			//~ {
			...
			//~ 'Phone' => $this->input->post('txt_state'),
			//~ 'Phone' => $this->input->post('txt_city'),
			//~ 'Phone' => $this->input->post('txt_landmark'),
			//~ 'Phone' => $this->input->post('txt_host_no'),
			//~ 'Phone' => $this->input->post('txt_pincode'),
			//~ 'Phone' => $this->input->post('txt_address'),
			//~ 'Phone' => $this->input->post('txt_landmark'),
		...
		//~ 'first_name' => $this->input->post('txt_empname'),
		//~ 'user_type' => $this->input->post('txt_utype'),
		//~ 'address' => $this->input->post('txt_phone'),
		//~ 'Phone' => $this->input->post('txt_emp_addr'),
		...
		//~ if($this->Employee_Model->insertEmployee($data)){           
			...
			//~ {
				...
				//~ echo   '<div class="alert alert-success text-center">Successfully registered. Please confirm the mail that has been sent to your email. </div>';
        ...
        public function unquie_fro_val()
	{
       ...
       public function user_rating()
		...
		$ratdata =	array('po_id'=>$this->input->post('poid'),'comid'=>$this->input->post('comid'),'userid'=>$this->input->post('curruid'),'countrating'=>$this->input->post('count'));
