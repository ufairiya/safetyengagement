...
class Users_model extends CI_Model{
	...
	public function get_compratingjob($ratedata)
	{
		$this->db->select('*');
		$this->db->from('ratingcomp');
		$this->db->where('po_id',$ratedata['po_id']);
		//~ $this->db->where('comid',$ratedata['userid']);
		$this->db->where('usid',$ratedata['usid']);
		$query = $this->db->get();
		return $result = $query->row();
	}
	public function get_compratingjobprofile($ratedata)
	{
		$this->db->select('*');
		$this->db->from('ratingcomp');
		//~ $this->db->where('comid',$ratedata['userid']);
	...
	public function insertratingcompany($ratedata)
	{
		...
		if(!empty($result))
		{
			$this->db->where('compra_id',$result->compra_id);
			return  $this->db->update('ratingcomp',array('countrating'=>$ratedata['countrating'])); 
		}
		else
		{			
			return  $this->db->insert('ratingcomp',$ratedata); 
		}
	}
       ...
       public function get_bidjob($uid)
	{
		...
		return $result = $query->result();
	       ...
	       public function get_usermastercompany()
	{
		$this->db->select('*');
		$this->db->from('user_master');
		$this->db->where('user_type','company');
