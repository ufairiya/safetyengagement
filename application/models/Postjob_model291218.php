...
class Postjob_model extends CI_Model{
	...
	public function get_postjoballpaidfilter($filter)
	{
			...
			$this->db->select('proposal.proposproj_id,postjob.state,postjob.city,postjob.po_id,postjob.highleveljobdesc,postjob.job_description,postjob.budget,postjob.explevel1,postjob.joblocation,postjob.hourorfix,postjob.posteddate,postjob.company_userid,postjob.job_status,user_master.status as umstatus');
			$this->db->from('postjob');
			$this->db->join('user_master' ,'user_master.id_user_master=postjob.company_userid');
			$this->db->join('proposal','proposal.proposproj_id = postjob.po_id','left');
			$this->db->where('postjob.job_status',2);
