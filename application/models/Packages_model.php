...
class Packages_model extends CI_Model{
	...
	public function get_checkfree_package()
	{
		$this->db->select('*');
		$this->db->from('subscription_package');
		$this->db->where('pkg_type ','free');
	...
	public function getpackagedetailsdata()
	{
		$this->db->select('*');
		$this->db->from('subscription_package');
		$this->db->where('user_id',$this->session->userdata('id_user_master'));
		$this->db->where('package_count !=',0);
		$this->db->where('status','1');
		$query =$this->db->get();
		$result = $query->row();
		return $result;
	...
	public function getallpackagedetailsdata()
	{
		$this->db->select('*');
		$this->db->from('subscription_package');
		$this->db->where('user_id',$this->session->userdata('id_user_master'));
	...
	public function n_free_package()
	{
		$this->db->select('*');
		$this->db->from($this->table);
		$this->db->where('pkg_amt !=',0);
		$query =$this->db->get();
		$result = $query->result();
		return $result;
	}
