...
class Postjob_model extends CI_Model{
	...
	public function getcompanypostdata($jobid)
		{
			$this->db->select('postjob.*,user_master.*, postjob.industry as industryname, postjob.insurance as projinsurence , postjob.city as citypost , postjob.state as statepost', false);
			$this->db->from('postjob');
	...
	public function updateemptystatus($p_id,$data)
		{
		$this->db->where('id !=',$p_id);
			$this->db->where('user_id',$this->session->userdata('id_user_master'));
			$this->db->update('subscription_package', $data);	
	...
	public function updatefreestatus($p_id,$data)
		{
			
				$this->db->where('id',$p_id);
			$this->db->where('user_id',$this->session->userdata('id_user_master'));
			$datares = $this->db->update('subscription_package', $data);	
	...
	public function get_postjoball()
		{
			$this->db->select('postjob.state,postjob.city,postjob.po_id,postjob.highleveljobdesc,postjob.job_description,postjob.budget,postjob.explevel1,postjob.joblocation,postjob.hourorfix,postjob.posteddate,postjob.company_userid,postjob.job_status,user_master.status as umstatus');
	...
	public function get_postjoballdetails()
		{
			...
			$this->db->select('postjob.job_title,postjob.state,postjob.city,postjob.po_id,postjob.highleveljobdesc,postjob.jobemergency,postjob.job_description,postjob.budget,postjob.explevel1,postjob.joblocation,postjob.hourorfix,postjob.posteddate,postjob.company_userid,postjob.job_status,user_master.status as umstatus');
	...
	public function get_postjoballdetailsdata()
		{
	...
	public function get_postjoballpaidfilter($filter)
	{
			...
			//~ $this->db->select('postjob.*,user_master.*,proposal.*,user_master.status as umstatus');
			$this->db->select('proposal.proposproj_id,postjob.job_title,postjob.state,postjob.city,postjob.po_id,postjob.highleveljobdesc,postjob.jobemergency,postjob.job_description,postjob.budget,postjob.explevel1,postjob.joblocation,postjob.hourorfix,postjob.posteddate,postjob.company_userid,postjob.job_status,user_master.status as umstatus');
			$this->db->from('postjob');
			$this->db->join('user_master' ,'user_master.id_user_master=postjob.company_userid');
			$this->db->join('proposal','proposal.proposproj_id = postjob.po_id','left');
			$this->db->where('postjob.job_status',2);
			$this->db->distinct('postjob.po_id');
			$this->db->where("( postjob.state LIKE '%".$filter['finddata']."%' OR  postjob.zipcode LIKE '%".$filter['finddata']."%' OR  postjob.highleveljobdesc LIKE '%".$filter['finddata']."%' OR  postjob.job_description LIKE '%".$filter['finddata']."%' OR  postjob.industry LIKE '%".$filter['finddata']."%' OR  postjob.internship LIKE '%".$filter['finddata']."%')", NULL, FALSE);
			$this->db->order_by('postjob.po_id','desc');
			$query = $this->db->get();
			return $result = $query->result();
			 ...
			 //~ $this->db->select('proposal.proposproj_id,postjob.state,postjob.po_id,postjob.highleveljobdesc,postjob.job_description,postjob.budget,postjob.explevel1,postjob.joblocation,postjob.hourorfix,postjob.posteddate,postjob.company_userid,postjob.job_status');
			//~ $this->db->from('postjob');
			//~ $this->db->join('proposal','proposal.proposproj_id = postjob.po_id');
			//~ $this->db->distinct('postjob.po_id');
			 //~ $this->db->where('postjob.job_status',2);
			//~ $this->db->where('LOWER(postjob.state)',strtolower($fullstate));
			//~ $this->db->order_by('postjob.po_id','desc');
			//~ $query = $this->db->get();
	...
	public function get_postmanagejoball($usid)
		{
			...
			$this->db->select('postjob.*,user_master.*, postjob.city as citypost , postjob.state as statepost', false);
			$this->db->from('postjob');
			$this->db->join('user_master' ,'user_master.id_user_master=postjob.company_userid');
	...
	public function get_postdraftjoball($usid)
		{
			...
			$this->db->join('user_master' ,'user_master.id_user_master=postjob.company_userid');
			$this->db->where('postjob.company_userid',$usid);
			$this->db->where('postjob.job_status',0);
			$this->db->order_by('postjob.po_id','desc');
			$query = $this->db->get();
			return $result = $query->result();
		}
	...
	public function get_postproposals($usid)
		{
			$this->db->select('postjob.*,user_master.id_user_master,user_master.first_name,user_master.profile_image');
			$this->db->from('postjob');
			$this->db->join('user_master' ,'user_master.id_user_master=postjob.company_userid');
	...
	public function get_postprosalposttable($poid)
		{
			$this->db->distinct();
			$this->db->select('proposproj_id,status');
			$this->db->from('proposal');
			...
			//~ $this->db->or_where('proposal.status',5);
			$this->db->where('postjob.job_status',2);
			$query = $this->db->get();
			return $result = $query->result();
	...
	public function get_postawardedtable($uid)
		{
			$this->db->distinct();
			$this->db->select('proposproj_id,status');
			$this->db->from('proposal');
			$this->db->join('postjob','postjob.po_id = proposal.proposproj_id');
			$this->db->where('proposal.comp_id',$uid);
			//~ $this->db->where('proposal.status',2);
			$this->db->where('postjob.job_status',3);
			$this->db->where('proposal.status',2);
			$this->db->or_where('proposal.status',5);
	...
	public function update_awarded_job($p_id,$data)
		{
			...
			public function get_mapsearchdataall($fullstate)
		{
			
			$this->db->select('postjob.job_title,postjob.state,postjob.city,postjob.po_id,postjob.highleveljobdesc,postjob.jobemergency,postjob.job_description,postjob.budget,postjob.explevel1,postjob.joblocation,postjob.hourorfix,postjob.posteddate,postjob.company_userid,postjob.job_status,user_master.status as umstatus');
			$this->db->distinct('po_id');
 ...
 $this->db->where('job_status',2);
	...
	public function get_mapsearchdata($fullstate)
		{
		 $this->db->select('proposal.proposproj_id,postjob.city,postjob.state,postjob.po_id,postjob.highleveljobdesc,postjob.job_description,postjob.budget,postjob.explevel1,postjob.joblocation,postjob.hourorfix,postjob.posteddate,postjob.company_userid,postjob.job_status');
			$this->db->from('postjob');
			$this->db->join('proposal','proposal.proposproj_id = postjob.po_id');
			$this->db->distinct('postjob.po_id');
			 $this->db->where('postjob.job_status',2);
	...
	public function get_postjobpaiduser($user_id)
		{
			$this->db->select('postjob.*,user_master.profile_image,user_master.first_name');
			$this->db->from('postjob');
			$this->db->join('user_master' ,'user_master.id_user_master=postjob.company_userid');
	...
	public function get_post_award_p($poid)
		{
		    ...
		    $this->db->where('proposal.status',2);
			$this->db->or_where('proposal.status',5);
			//~ $this->db->or_where('proposal.status',1);
			$query = $this->db->get();
			return $result = $query->result();
		
		}
	public function get_post_award_sentoffer($poid)
		{
			$this->db->select('proposal.*,user_master.*,proposal.status as prostatus');
			$this->db->from('proposal');
			$this->db->join('user_master','user_master.id_user_master = proposal.proposuser_id');
		    $this->db->where('proposal.proposproj_id',$poid);
			$this->db->where('proposal.comp_id',$this->session->userdata('id_user_master'));
		    $this->db->where('proposal.status',5);
			 $this->db->or_where('proposal.status',1);
			$query = $this->db->get();
			return $result = $query->result();
	...
	public function	check_awardedjob1($p_id,$comuid)
		{
			...
			public function	check_sfconfirmawardjob($p_id,$comuid)
		{
			$this->db->select('*');
			$this->db->from('proposal');
			$this->db->where('proposproj_id',$p_id);
	...
	public function getuserawarddata($uid)
		{
			$this->db->select('*');
			$this->db->from('user_master');
	...
	public function getuserallondata()
		{
			$this->db->select('*');
			$this->db->from('user_master');
			$this->db->where('notifionoff','on');
			$this->db->where("(user_type='professional' OR user_type='student')");
			$this->db->where('status',1);
			$query = $this->db->get();
			return $result = $query->result();
}
	public function getuserprofessionondata()
		{
			$this->db->select('*');
			$this->db->from('user_master');
			$this->db->where('notifionoff','on');
	...
	public function checkpoidjob($poid)
		{
			$this->db->select('*');
			$this->db->from('postjob');
			$this->db->where('po_id',$poid);
	...
	public function insertindustry($insertname)
		{
		$this->db->insert('industry', array('industry_name'=>$insertname,'tag'=>strtolower($insertname),'status'=>1));	
			return $this->db->insert_id();
		}
			
	public function get_p_job()
		{
			$this->db->select('*');
			$this->db->from('postjob');
			$this->db->where('job_status',2);
	...
	public function update_expired_pro($p_id,$data,$pdate)
		{
			...
			else
			{
			 ...
			 $new_date = date('m/d/y 00:00:00', strtotime(date('m/d/y 00:00:00'). " + 20 days"));
			$this->db->select('*');
			$this->db->from('postjob');
			$this->db->where('posteddate >',$new_date);
			$this->db->where('po_id',$p_id);
	...
	public function getjobdetails($poid)
		{
			$this->db->select('*');
			$this->db->from('postjob');
			$this->db->where('po_id',$poid);
