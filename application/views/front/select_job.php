...
<section>
	<div class="block">
		<div class="container">
			<div class="row">
				<div class="col-lg-8 col-md-12 col-sm-12 column jobdetailslt">
					<div class="job-single-sec">
						<div class="job-single-head">
							<?php $pro_img = $view_job->profile_image != "" ? $view_job->profile_image : 'default17.jpg'  ?>
							<div class="job-thumb"> <img src="<?php echo $base_url.'uploads/'.$pro_img; ?>" alt="" style="width:100px"> </div>
...
-->
						...
						<div class="job-details">
							...
							<?php  }if(!empty($get_job->equipment_pic))
							{
								...
								if($get_job->safetyhazards	){ ?>
							<h3>Hazards and safety requirements	</h3>
							<p><?php echo $get_job->safetyhazards	 ?></p>
							<?php }
					...
					<div class="col-lg-4 col-md-12  col-sm-12 column jobdetailsrt">
				<div class="job-single-sec">
					<?php 
					$check_awardedjob = $this->postjob_model->check_awardedjob1($postjobid,$view_job->id_user_master);
					$check_sfcfawardedjob = $this->postjob_model->check_sfconfirmawardjob($postjobid,$view_job->id_user_master);
					//~ echo "<pre>";
					//~ var_dump($check_sfcfawardedjob);
...
if(empty($check_awardedjob)){
						if(!empty($this->session->userdata('user_type_fr')) ) 
					{
						...
						<?php }
						else  if($datadetailpayment->status ==  2 && $get_job->job_status == 2)
						{ ?>
						<button type="button" class="apply-thisjob btn " disabled>Proposal Sent</button>
						<?php }
						else  if($datadetailpayment->status ==  2 && $get_job->job_status == 5)
						{ ?>
						<button type="button" class="apply-thisjob btn " disabled>Completed</button>
<!--
								  ...
								  <div class="control-label ">
									 ...
									 <div class="col-50">
										<label for="fname" >
										   <div class="form-group" id="ratingcompany"> </div>
										   <span class="review-text" style="display:none"><span id="starCountcompany"></span> rating</span>
										</label>
									 </div>
								  </div>
							   </div>		          <input type="hidden"  class="getcount" name="getcount"  value="<?php if(!empty($get_compratingjob->countrating)){ echo $get_compratingjob->countrating; } ?>" />
-->

							 <input type="hidden" id="sfuid" class="sfuid" value="<?php echo $this->session->userdata('id_user_master'); ?>">

					<?php }
					
						else if($check_sfcfawardedjob->status == 5 && $get_job->job_status == 3)
						{ ?>
<!--
						<a href="<?php echo $base_url?>job/cfaward_sf?pid=<?php echo $get_job->po_id; ?>"> <button type="button" class="apply-thisjob btn " >confirm</button></a>
-->
 <a href="<?php echo $base_url; ?>bids/cfaward_sf?proj_id=<?php echo $get_job->po_id; ?>"><button type="button" class="apply-thisjob btn " >Confirm Offer</button></a>
<!--
						<form id="select_job" action="<?php echo $base_url?>job/cfaward_sf" name="select_job" method="POST" >
						<p class="apply-thisjob">  <input type="submit" name="submit_job" value="Confirm Job" id="submit_job"></p> 
						...
						<input type="hidden" id="applyuser" name="applyuser" value="<?php echo $this->session->userdata('email');?>"> 
						</form>
-->
...
-->
			 ...
			 <input type="hidden" id="sfuid" class="sfuid" value="<?php echo $user_job->id_user_master; ?>">
						...
						<?php 	 
								...
								<div class="panel-heading awardjob"><p class="panel-title text-center" style="color: #ee5f1c;font-size: 33px;">Awarded</p></div>
						<?php }   } ?>
						...
						<ul>
							...
							<li><h3>Internship</h3><span><?php  if(!empty($get_job->internship)){ echo "Yes"; }else{ echo "No"; } ?></span></li>
...
<script src="<?php echo $base_url; ?>assets/js/jquery.emojiRatings.min.js"></script>
                  ...
                  <script>
                     $("#ratingcompany").emojiRating({
                     	fontSize: 32,
                     	onUpdate: function(count) {
                     		$(".review-text").show();
                     		$("#starCountcompany").html(count + " Star");
                     		var poid = $('.poid').val();	
                     			var comid = $('.comid').val();
                     			var sfuid = $('.sfuid').val();
                     			...
                     			$.ajax({
                     				url: '<?php echo $base_url?>signup_controller/user_ratingcompany',
                     				data:{poid,comid,sfuid,count},
                     				type: 'POST',
                     				success:function(data)
                     				{ 
