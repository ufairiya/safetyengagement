...
class Job extends CI_Controller {
	...
	public function index()
	{
		...
		if($this->session->userdata('user_type_fr') == 'company')
		...
		else
			{
				...
				$this->session->set_flashdata('showpopup','login');
				redirect('home');
			}	
	...
	{  
		...
		if(!empty($this->input->post('saveasdraft')))
		{
			$job_status ="0";
		}
		else{
			...
			if(!empty($this->input->post('industry')))
			{
				$insertindustry = $this->postjob_model->insertindustry($this->input->post('industry'));
				$industry_c = (!empty($insertindustry)) ? $insertindustry :"";

			}
			else
			{			$industry_c = (!empty($this->input->post('se_industry'))) ? $this->input->post('se_industry') :"";

			}
			
			$postjobdata = array('job_title'=>$this->input->post('job_title'),'highleveljobdesc'=>$this->input->post('highleveljobdesc'),'job_description'=>$this->input->post('job_description'),'equipment'=>$this->input->post('equipment'),'finalrep'=>$this->input->post('finalrep'),'industry'=>$industry_c,'internship'=>$internship,'project'=>$this->input->post('project'),'start_date'=>$this->input->post('start_date'),'end_date'=>$this->input->post('end_date'),'jobemergency'=>$this->input->post('jobemergency'),'worktype_desc'=>json_encode($this->input->post('worktype_desc')),'explevel1'=>$this->input->post('explevel1'),'thirdpartapprov'=>$thirdpartapprov,'thirdpartother'=>$othertxt,'joblocation'=>$this->input->post('administrative_area_level_1'),'insurance'=>$this->input->post('jobinsurence'),'job3party'=>$this->input->post('job3party'),'city'=>$this->input->post('locality'),'state'=>$this->input->post('administrative_area_level_1'),'zipcode'=>$this->input->post('postal_code'),'insurance'=>$insurance,'hourorfix'=>$hourorfix,'budget'=>$this->input->post('budget'),'detailed_pic'=>$file_p1,'safetyhazards'=>$this->input->post('safetyhazards'),'equipment_pic'=>$file_p2,'finalrep_pic'=>$file_p3,'project_pic'=>$file_p4,'certificates_pic'=>$file_p5,'insurance_certificate'=>$file_p6,'budget_img'=>$file_p7,'job_file'=>json_encode($file_p8),'posteddate'=>date('m/d/Y h:i:s'),'company_userid'=>$this->session->userdata('id_user_master'),'job_status'=>$job_status);
			...
			{
				$userdata = $this->postjob_model->getuserallondata();

			}
			else
			{
				$userdata = $this->postjob_model->getuserprofessionondata();
			}
			
			foreach($userdata as $userdataval )
			{
				
				$userdatamail[] = $userdataval->email;
				
			}
			
			 $userdatamailimp = implode(",",$userdatamail);
			
		if(!empty($last_insert_id))
		{			
		if(!empty($this->input->post('saveasdraft')))
		{
			$this->session->set_flashdata('showsuccess','Successfully Draft Job');
			redirect(base_url()."job/draft_jobposts");
									
		}else{
			
			$this->load->library('email');
			$subject = 'New Job Posted | Satety Engagement';
			$message = '<p>Thank you for new job posted .</p>';
			// Get full html:
			$body = '<!DOCTYPE html >
			<html >
			<head>
			...
			$result = $this->email
			->from('omaraw1965@gmail.com')
			->cc('test.stallioni@gmail.com,surya@stallioni.com')    
			->to($userdatamailimp)
			->subject($subject)
			->message($body)
			->send();
		$this->session->set_flashdata('showsuccess','Successfully Awarded Job');
		...
		{
			$this->session->set_flashdata('showerror','Unsuccessfully Awarded Job ');
			redirect('job');
		}
	}
...
//~ echo "<pre>";
		...
		//~ exit;
			...
			$thirdpartapprov = (!empty($this->input->post('thirdpartapprov'))) ? $this->input->post('thirdpartapprov') : '' ;
			$othertxt = $this->input->post('thirdpartapprov') == 4 ? $this->input->post('othertxt') : '' ;
			//~ $industry =  (!empty($this->input->post('industryinp'))) ? $this->input->post('industryinp') : $this->input->post('industry') ;
			if(!empty($this->input->post('industry')))
			{
				$insertindustry  = $this->postjob_model->insertindustry($this->input->post('industry'));
				$industry_c = (!empty($insertindustry)) ? $insertindustry :"";
			...
			$postjobdata = array('job_title'=>$this->input->post('job_title'),'highleveljobdesc'=>$this->input->post('highleveljobdesc'),'job_description'=>$this->input->post('job_description'),'equipment'=>$this->input->post('equipment'),'safetyhazards'=>$this->input->post('safetyhazards'),'finalrep'=>$this->input->post('finalrep'),'information'=>$this->input->post('information'),'industry'=>$industry_c,'internship'=>$internship,'project'=>$this->input->post('project'),'start_date'=>$this->input->post('start_date'),'end_date'=>$this->input->post('end_date'),'jobemergency'=>$this->input->post('jobemergency'),'worktype_desc'=>json_encode($this->input->post('worktype_desc')),'explevel1'=>$this->input->post('explevel1'),'thirdpartapprov'=>$thirdpartapprov,'thirdpartother'=>$othertxt,'joblocation'=>$this->input->post('administrative_area_level_1'),'city'=>$this->input->post('locality'),'state'=>$this->input->post('administrative_area_level_1'),'zipcode'=>$this->input->post('postal_code'),'insurance'=>$insurance,'hourorfix'=>$this->input->post('hourorfix'),'budget'=>$this->input->post('budget'),'detailed_pic'=>implode(",",array_filter($detailed_p)),'equipment_pic'=>implode(",",array_filter($equipment_p)),'finalrep_pic'=>implode(",",array_filter($finalrep_p)),'project_pic'=>implode(",",array_filter($project_p)),'certificates_pic'=>implode(",",array_filter($certificates_p)),'insurance_certificate'=>implode(",",array_filter($insurance_c)),'budget_img'=>implode(",",array_filter($budget_p)),'job_file'=>json_encode(explode(",",$this->input->post('files_pic'))),'posteddate'=>date('m/d/Y h:i:s'),'company_userid'=>$this->session->userdata('id_user_master'),'job_status'=>$job_status );
...
{   
	...
	public function jobposts()
	{
		...
		//~ echo "<pre>";
		//~ var_dump($data['get_postawardedtable']);
...
//~ exit;
	...
	public function draft_jobposts()
	{

		$get_postdraftjoball = $this->postjob_model->get_postdraftjoball($this->session->userdata('id_user_master'));
	
		$data['get_postjobexpired']  =   $this->postjob_model->get_postjobexpired($this->session->userdata('id_user_master'));
		$data['base_url'] = base_url();
		$data['get_postjoball']  =   $get_postdraftjoball;
		$data['current_menu']  = "job";
		$data['sub_menu']  = "job";
		$data['view_file']  = "draftjobs";
		$this->template->load_front_home_template($data);		
	}
	...
	public function award_sf()
	{
        ...
        $userdata = $this->postjob_model->getuserawarddata($this->input->get('user_id'));

		//~ $this->postjob_model->update_awarded_otherstatus_job($this->input->get('proj_id'),array('status' => 3));
		$this->postjob_model->update_awarded_job($datasendpro,array('status' => 5));
		
		$dataaward =	array('job_status' => 3);
		
		$last_update = $this->postjob_model->update_awarded_post($p_id,$dataaward);
						...
						if(!empty($last_update))
									{		
											...
											$this->load->library('email');
											$subject = 'Successfully Awarded Job | Satety Engagement';
											$message = '<p>Thank you for Awarded job by Safety Engagement.</p>';
											// Get full html:
											$body = '<!DOCTYPE html >
											<html >
											<head>
	...
	public function select_job($postjob = 0 )
	{
		...
		if($user_id)
			...
			{
				$this->session->set_flashdata('showpopup','message');
				redirect('home');
			}
	}
	...
	public function complete_job($poid = "")
	{
		...
		$checkpoidjob = $this->postjob_model->checkpoidjob($poid);
		if(!empty($checkpoidjob) && $checkpoidjob->job_status == 5 )
		{
		...
		$getrating = $this->users_model->get_ratingjob(array(
			...
			$data['get_compratingjob'] = !empty($get_compratingjob) ? $get_compratingjob : '';
		$data['base_url'] = base_url();
		$data['poid'] = $poid;
		$data['current_menu']  = "job";
		$data['sub_menu']  = "job";
