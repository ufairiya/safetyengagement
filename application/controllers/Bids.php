...
class Bids extends CI_Controller {
	...
	public function find_job($fullstate = "")
	{
		...
		if(!empty($fullstate))
		{
			$get_postjoballarrpush = $this->postjob_model->get_mapsearchdataall($fullstate);
...
$data['get_bid_job']  =   $get_bidjoballpaid;
	...
	public function send_proposals()
	{
			if($this->session->userdata('user_type_fr') == 'professional' || $this->session->userdata('user_type_fr') == 'student')
			{
				...
				else
				{
					...
					$proposal_filesdataattach = $proposal_files != "" ? base_url().'uploads/'.$proposal_files["upload_data"]['file_name'] :  "";
					...
					$last_insert = $this->bids_model->insert_sendproposal($datasendprodata);
					...
					$this->postjob_model->update_postproposal($pro_id,array('job_status' => '2'));
					        ...
					        $userdata = $this->postjob_model->getuserawarddata($this->input->post('comuser_id'));

			$this->load->library('email');
			$subject = 'Sent Proposal For You  | Satety Engagement';
			$message = '<p>Thank you for sent proposal job by Safety Engagement.</p>';
			// Get full html:
			$body = '<!DOCTYPE html >
			<html >
			<head>
			...
			->subject($subject)
			->message($body)
			 ->attach($proposal_filesdataattach)
			->send();
	...
	public function select_job($postjob = 0 )
	{
		...
		$data['get_compratingjob'] = $this->users_model->get_compratingjob(array('usid' => $user_id,'po_id' => $postjob));
		...
		//~ print_r($data['get_job'] );
		//~ exit;
		if(!empty($data['get_job'] ))
		{
		$city = $data['get_job']->city;
		if(!empty($city)){
			...
			if($this->session->userdata('user_type_fr') == 'professional' || $this->session->userdata('user_type_fr') == 'student')
				{ 
					...
					if(!empty($get_subscription_package))
							...
							if(empty($check_bidins ))
								{
									...
									$package_count = (0 < $get_subscription_package->package_count) ? $get_subscription_package->package_count - 1 : 0;
						...
						{
							$this->session->set_flashdata('showpopup','insertjob login');
							redirect('bids/select_job/'.$this->input->post('post')); 
						}	
				}
			else
	...
	public function stripe_payment()
	{
		...
		$pro_amt = round($pro_amt1);
			...
			$uptempdata = array("status" => '3',"package_count" => 0 );
			$updateempty = $this->postjob_model->updateemptystatus($itemNumber,$uptempdata);
	...
	public function completejob($poid = 0)
	{
	...
	public	function updatefreebid($id = 0)
	{
		$uptdata = array("status" => 1,"pkg_type" => "free");
		$updatesucess = $this->postjob_model->updatefreestatus($id, $uptdata);
		if ($updatesucess != false)
		{
			redirect('bids/select_job/'.$id);
		}
		else
		{
			redirect('home');
	...
	public function cfaward_sf()
	{
		
	
		$datasendpro =	array('proposproj_id' => $this->input->get('proj_id') ,'proposuser_id' =>$this->session->userdata('id_user_master'));
		$p_id = $this->input->get('proj_id');
		
        $userdata = $this->postjob_model->getuserawarddata($this->session->userdata('id_user_master'));
